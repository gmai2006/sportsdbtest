...
public class DefaultMediaDao implements MediaDao {
    ...
    @Inject
    @Named("DefaultJpaDao")
    public DefaultMediaDao(JpaDao dao) {
        this.dao = dao;
    }
    ...
    /** {@inheritDoc} */
    @Override
    public List<Media> select(int max) {
        return dao.select("select a from Media a", Media.class, max);
    }

    /** {@inheritDoc} */
    @Override
    public List<Media> selectAll() {
        return dao.selectAll("select a from Media a", Media.class);
    }
