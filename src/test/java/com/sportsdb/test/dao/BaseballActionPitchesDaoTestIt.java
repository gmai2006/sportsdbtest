...
public class BaseballActionPitchesDaoTestIt {
  static final String inputFile = "BaseballActionPitches.json";
  static BaseballActionPitchesDao dao;
  static Gson gson =
      new GsonBuilder()
  ...
  public static void beforeClass() {
    ...
    dao = new DefaultBaseballActionPitchesDao(jpa);
  }
  ...
  public void testSelect() {
    BaseballActionPitches testResult = dao.find(records[1].getId());
    assertNotNull("expect result", testResult);
    org.junit.Assert.assertTrue(
        "expect equals baseballActionPlayId ",
        this.records[1].getBaseballActionPlayId() == testResult.getBaseballActionPlayId());
    org.junit.Assert.assertTrue(
    ...
    org.junit.Assert.assertEquals(
        "expect equals umpireCall ", this.records[1].getUmpireCall(), testResult.getUmpireCall());
    org.junit.Assert.assertEquals(
        "expect equals pitchLocation ",
        this.records[1].getPitchLocation(),
        testResult.getPitchLocation());
    org.junit.Assert.assertEquals(
        "expect equals pitchType ", this.records[1].getPitchType(), testResult.getPitchType());
    org.junit.Assert.assertTrue(
        "expect equals pitchVelocity ",
        this.records[1].getPitchVelocity() == testResult.getPitchVelocity());
    org.junit.Assert.assertEquals(
        "expect equals comment ", this.records[1].getComment(), testResult.getComment());
    org.junit.Assert.assertEquals(
        "expect equals trajectoryCoordinates ",
        this.records[1].getTrajectoryCoordinates(),
        testResult.getTrajectoryCoordinates());
    org.junit.Assert.assertEquals(
        "expect equals trajectoryFormula ",
        this.records[1].getTrajectoryFormula(),
        testResult.getTrajectoryFormula());
    org.junit.Assert.assertEquals(
        "expect equals ballType ", this.records[1].getBallType(), testResult.getBallType());
    org.junit.Assert.assertEquals(
        "expect equals strikeType ", this.records[1].getStrikeType(), testResult.getStrikeType());
  }
}
