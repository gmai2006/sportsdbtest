...
import com.sportsdb.test.dao.DefaultWageringTotalScoreLinesDao;
...
public class WageringTotalScoreLinesHandlerTestIt {
  static final String inputFile = "WageringTotalScoreLines.json";
  static WageringTotalScoreLinesHandler handler;
  ...
  static Gson gson =
      new GsonBuilder()
          ...
          .create();
  private WageringTotalScoreLines[] records;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/WageringTotalScoreLines.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, WageringTotalScoreLines[].class);
    assertEquals("match count", count, records.length);
    WageringTotalScoreLines testResult =
        jpa.find(WageringTotalScoreLines.class, records[0].getId());
    ...
    assertEquals(
        "expect equals lineOver ", this.records[0].getLineOver(), testResult.getLineOver());
    assertEquals(
        "expect equals lineUnder ", this.records[0].getLineUnder(), testResult.getLineUnder());
    assertEquals("expect equals total ", this.records[0].getTotal(), testResult.getTotal());
    assertEquals(
        "expect equals totalOpening ",
        this.records[0].getTotalOpening(),
