...
import com.sportsdb.test.dao.DefaultTeamPhasesDao;
...
public class TeamPhasesHandlerTestIt {
  static final String inputFile = "TeamPhases.json";
  static TeamPhasesHandler handler;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/TeamPhases.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, TeamPhases[].class);
    assertEquals("match count", count, records.length);
    TeamPhases testResult = jpa.find(TeamPhases.class, records[0].getId());
    ...
    assertEquals(
        ...
        this.records[0].getStartDateTime(),
        testResult.getStartDateTime());
    assertEquals(
        "expect equals endDateTime ",
        this.records[0].getEndDateTime(),
