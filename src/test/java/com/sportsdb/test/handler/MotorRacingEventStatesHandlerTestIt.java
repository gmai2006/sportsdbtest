...
import com.sportsdb.test.dao.DefaultMotorRacingEventStatesDao;
...
public class MotorRacingEventStatesHandlerTestIt {
  static final String inputFile = "MotorRacingEventStates.json";
  static MotorRacingEventStatesHandler handler;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/MotorRacingEventStates.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, MotorRacingEventStates[].class);
    assertEquals("match count", count, records.length);
    MotorRacingEventStates testResult = jpa.find(MotorRacingEventStates.class, records[0].getId());
    ...
    assertEquals("expect equals lap ", this.records[0].getLap(), testResult.getLap());
    assertEquals(
        "expect equals lapsRemaining ",
        this.records[0].getLapsRemaining(),
        testResult.getLapsRemaining());
    assertEquals(
        "expect equals timeElapsed ",
        this.records[0].getTimeElapsed(),
        testResult.getTimeElapsed());
    assertEquals(
        "expect equals flagState ", this.records[0].getFlagState(), testResult.getFlagState());
