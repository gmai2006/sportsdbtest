...
import com.sportsdb.test.dao.DefaultIceHockeyEventStatesDao;
...
public class IceHockeyEventStatesHandlerTestIt {
  static final String inputFile = "IceHockeyEventStates.json";
  static IceHockeyEventStatesHandler handler;
  ...
  static Gson gson =
      new GsonBuilder()
          ...
          .create();
  private IceHockeyEventStates[] records;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/IceHockeyEventStates.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, IceHockeyEventStates[].class);
    assertEquals("match count", count, records.length);
    IceHockeyEventStates testResult = jpa.find(IceHockeyEventStates.class, records[0].getId());
