...
import com.sportsdb.test.dao.DefaultBasketballEventStatesDao;
...
public class BasketballEventStatesHandlerTestIt {
  static final String inputFile = "BasketballEventStates.json";
  static BasketballEventStatesHandler handler;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/BasketballEventStates.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, BasketballEventStates[].class);
    assertEquals("match count", count, records.length);
    BasketballEventStates testResult = jpa.find(BasketballEventStates.class, records[0].getId());
