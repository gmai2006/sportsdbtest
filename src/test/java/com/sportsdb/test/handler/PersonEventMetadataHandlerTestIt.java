...
import com.sportsdb.test.dao.DefaultPersonEventMetadataDao;
...
public class PersonEventMetadataHandlerTestIt {
  static final String inputFile = "PersonEventMetadata.json";
  static PersonEventMetadataHandler handler;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/PersonEventMetadata.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, PersonEventMetadata[].class);
    assertEquals("match count", count, records.length);
    PersonEventMetadata testResult = jpa.find(PersonEventMetadata.class, records[0].getId());
    ...
    assertEquals("expect equals health ", this.records[0].getHealth(), testResult.getHealth());
    ...
    org.junit.Assert.assertEquals(
        "expect equals lineupSlot ", this.records[0].getLineupSlot(), testResult.getLineupSlot());
    org.junit.Assert.assertEquals(
        "expect equals lineupSlotSequence ",
        this.records[0].getLineupSlotSequence(),
        testResult.getLineupSlotSequence());
