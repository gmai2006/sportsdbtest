...
import com.sportsdb.test.dao.DefaultWageringMoneylinesDao;
...
public class WageringMoneylinesHandlerTestIt {
  static final String inputFile = "WageringMoneylines.json";
  static WageringMoneylinesHandler handler;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/WageringMoneylines.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, WageringMoneylines[].class);
    assertEquals("match count", count, records.length);
    WageringMoneylines testResult = jpa.find(WageringMoneylines.class, records[0].getId());
