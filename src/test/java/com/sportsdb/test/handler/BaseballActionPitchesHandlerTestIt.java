...
import com.sportsdb.test.dao.DefaultBaseballActionPitchesDao;
...
public class BaseballActionPitchesHandlerTestIt {
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/BaseballActionPitches.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, BaseballActionPitches[].class);
    assertEquals("match count", count, records.length);
    BaseballActionPitches testResult = jpa.find(BaseballActionPitches.class, records[0].getId());
    ...
    assertEquals(
        "expect equals umpireCall ", this.records[0].getUmpireCall(), testResult.getUmpireCall());
    assertEquals(
        "expect equals pitchLocation ",
        this.records[0].getPitchLocation(),
        testResult.getPitchLocation());
    assertEquals(
        "expect equals pitchType ", this.records[0].getPitchType(), testResult.getPitchType());
    org.junit.Assert.assertEquals(
    ...
    assertEquals(
        "expect equals strikeType ", this.records[0].getStrikeType(), testResult.getStrikeType());
