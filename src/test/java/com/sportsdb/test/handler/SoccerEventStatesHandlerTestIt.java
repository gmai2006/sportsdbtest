...
import com.sportsdb.test.dao.DefaultSoccerEventStatesDao;
...
public class SoccerEventStatesHandlerTestIt {
  static final String inputFile = "SoccerEventStates.json";
  static SoccerEventStatesHandler handler;
  ...
  static Gson gson =
      new GsonBuilder()
          ...
          .create();
  private SoccerEventStates[] records;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/SoccerEventStates.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, SoccerEventStates[].class);
    assertEquals("match count", count, records.length);
    SoccerEventStates testResult = jpa.find(SoccerEventStates.class, records[0].getId());
    ...
    assertEquals(
        ...
        this.records[0].getMinutesElapsed(),
        testResult.getMinutesElapsed());
    assertEquals(
        "expect equals periodMinuteElapsed ",
        this.records[0].getPeriodMinuteElapsed(),
