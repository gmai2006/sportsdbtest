...
import com.sportsdb.test.dao.DefaultStatsDao;
...
public class StatsHandlerTestIt {
  static final String inputFile = "Stats.json";
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/Stats.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, Stats[].class);
    assertEquals("match count", count, records.length);
    Stats testResult = jpa.find(Stats.class, records[0].getId());
    assertEquals(
        "expect equals statRepositoryType ",
        this.records[0].getStatRepositoryType(),
        testResult.getStatRepositoryType());
    org.junit.Assert.assertEquals(
        "expect equals statRepositoryId ",
        this.records[0].getStatRepositoryId(),
        testResult.getStatRepositoryId());
    assertEquals(
        "expect equals statHolderType ",
        this.records[0].getStatHolderType(),
        testResult.getStatHolderType());
    org.junit.Assert.assertEquals(
        "expect equals statHolderId ",
        this.records[0].getStatHolderId(),
        testResult.getStatHolderId());
    assertEquals(
        "expect equals statCoverageType ",
        this.records[0].getStatCoverageType(),
        testResult.getStatCoverageType());
    org.junit.Assert.assertEquals(
        "expect equals statCoverageId ",
        this.records[0].getStatCoverageId(),
        testResult.getStatCoverageId());
