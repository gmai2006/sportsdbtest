...
import com.sportsdb.test.dao.DefaultBaseballDefensiveStatsDao;
...
public class BaseballDefensiveStatsHandlerTestIt {
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/BaseballDefensiveStats.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, BaseballDefensiveStats[].class);
    assertEquals("match count", count, records.length);
    BaseballDefensiveStats testResult = jpa.find(BaseballDefensiveStats.class, records[0].getId());
