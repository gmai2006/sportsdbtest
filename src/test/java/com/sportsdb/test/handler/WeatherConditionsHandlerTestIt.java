...
import com.sportsdb.test.dao.DefaultWeatherConditionsDao;
...
public class WeatherConditionsHandlerTestIt {
  static final String inputFile = "WeatherConditions.json";
  static WeatherConditionsHandler handler;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/WeatherConditions.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, WeatherConditions[].class);
    assertEquals("match count", count, records.length);
    WeatherConditions testResult = jpa.find(WeatherConditions.class, records[0].getId());
    org.junit.Assert.assertEquals(
        "expect equals eventId ", this.records[0].getEventId(), testResult.getEventId());
    assertEquals(
        "expect equals temperature ",
        this.records[0].getTemperature(),
        testResult.getTemperature());
    assertEquals(
        "expect equals temperatureUnits ",
        this.records[0].getTemperatureUnits(),
        testResult.getTemperatureUnits());
    assertEquals(
        "expect equals humidity ", this.records[0].getHumidity(), testResult.getHumidity());
    assertEquals("expect equals clouds ", this.records[0].getClouds(), testResult.getClouds());
    assertEquals(
        "expect equals windDirection ",
        this.records[0].getWindDirection(),
        testResult.getWindDirection());
    assertEquals(
        "expect equals windVelocity ",
        this.records[0].getWindVelocity(),
        testResult.getWindVelocity());
    assertEquals(
        "expect equals weatherCode ",
        this.records[0].getWeatherCode(),
